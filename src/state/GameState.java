...
public class GameState extends BasicGameState {
    ...
    public void init(final GameContainer container, final StateBasedGame game)
        ...
        rockets[0] = new Player(0, 0, 0, rockets, this);
    ...
    public void render(GameContainer container, StateBasedGame game, Graphics g)
        ...
        drawBackgroud(x, y);
        Player player = (Player) rockets[0];
    ...
    public void update(final GameContainer container, final StateBasedGame game, int delta)
        ...
        if (!paused && !end) {
            ...
            player.bulletUpdate(dmgDoneNotifs);
            for (int i = 1; i < rockets.length; i++) {
                Npc npc = (Npc) rockets[i];
                if (npc.getHP() > 0) {
                    npc.update();
            ...
            if (input.isKeyPressed(Input.KEY_T) && player.getHP() == 0) {
            ...
            if (input.isKeyDown(Input.KEY_X) && player.getTarget() != null) {
                switch (player.getTarget().getTeam() % 4) {
                    case 0:
                        targetLabel.setColors(Color.blue);
                        break;
                    case 1:
                        targetLabel.setColors(Color.green);
                        break;
                    case 2:
                        targetLabel.setColors(Color.red);
                        break;
                    case 3:
                        targetLabel.setColors(Color.yellow);
                        break;
                }
            }
            ...
            energyLabels[0].setText(String.format("%02d",
                    Math.round(player.getWeaponEnergy() * 100))
                    + "%");
            energyLabels[1].setText(String.format("%02d",
                    Math.round((player.getEngineEnergy() < 0.02) ? 0
                            : player.getEngineEnergy() * 100))
                    + "%");
            energyLabels[2].setText(String.format("%02d",
                    Math.round(player.getShieldEnergy() * 100))
                    + "%");

            if (player.getTarget() != null) {
                float dist = player.getDistanceToTarget();
                arrowImage.setAlpha((dist < 400) ? 0 : ((dist >= 800) ? 1 : (dist - 400) / 400f));
                arrowImage.setRotation((float) (player.getAngleToTarget() * 180 / Math.PI));
                distanceLabel.setText(String.format("%.0f", dist));

                energyLabels[3].setText(String.format("%02d",
                        Math.round(player.getTarget().getWeaponEnergy() * 100))
                        + "%");
                energyLabels[4].setText(String.format("%02d",
                        Math.round(player.getTarget().getEngineEnergy() * 100))
                        + "%");
                energyLabels[5].setText(String.format("%02d",
                        Math.round(player.getTarget().getShieldEnergy() * 100))
                        + "%");
            }
        } else if (paused && !end) {
            if (input.isKeyPressed(Input.KEY_ESCAPE)) {
